...
function App() {
 ...
 const performRegression = (data) => {
   ...
   const residuals = regressionData.map((point, index) => {

     const predicted = slope * point[0] + intercept; // Predicted value from regression

     return point[1] - predicted; // Calculate residuals
   ...
   const residualSumOfSquares = residuals.reduce((sum, r) => sum + r ** 2, 0);

   const standardError = Math.sqrt(residualSumOfSquares / (data.length - 2)); // Standard error of the estimate




   const tStat = slope / (standardError / Math.sqrt(data.map(item => item.ethnicity).reduce((a, b) => a + b, 0) / (data.length - 1)));




   const df = data.length - 2; // degrees of freedom




   const pValue = 2 * (1 - jStat.studentt.cdf(Math.abs(tStat), df)); // Assuming jStat is included for statistical functions
   ...
   <button className={styles.uploadButton} onClick={() => window.location.href = '/'}>
     ...
     {lowMatchData.r > -1 && (
       ...
       <>
         ...
         <h4 className={styles.header3}>Low Match (0-30%) - R: {lowMatchData.r.toFixed(2)}, p-value: {lowMatchData.pValue}</h4>
     ...
     {mediumMatchData.r > -1 && (
       ...
       <>
         ...
         <h4 className={styles.header3}>Medium Match (30-70%) - R: {mediumMatchData.r.toFixed(2)}, p-value: {mediumMatchData.pValue}</h4>
     ...
     {highMatchData.r > -1 && (
       ...
       <>
         ...
         <h4 className={styles.header3}>High Match (70-100%) - R: {highMatchData.r.toFixed(2)}, p-value: {highMatchData.pValue}</h4>
